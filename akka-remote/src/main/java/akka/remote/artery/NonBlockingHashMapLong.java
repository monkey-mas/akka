...
// Copied from https://github.com/JCTools/JCTools/tree/master/jctools-experimental/src/main/java/org/jctools/maps
...
class NonBlockingHashMapLong<TypeV>
  ...
  private static final int _Obase  = Unsafe.instance.arrayBaseOffset(Object[].class);
  private static final int _Oscale = Unsafe.instance.arrayIndexScale(Object[].class);
  private static long rawIndex(final Object[] ary, final int idx) {
    assert idx >= 0 && idx < ary.length;
    return _Obase + idx * _Oscale;
  }
  private static final int _Lbase  = Unsafe.instance.arrayBaseOffset(long[].class);
  private static final int _Lscale = Unsafe.instance.arrayIndexScale(long[].class);
  ...
  static {                      // <clinit>
    ...
    _chm_offset = Unsafe.instance.objectFieldOffset(f);

    try { f = NonBlockingHashMapLong.class.getDeclaredField("_val_1"); }
    catch( java.lang.NoSuchFieldException e ) { throw new RuntimeException(e); }
    _val_1_offset = Unsafe.instance.objectFieldOffset(f);
  }
  private final boolean CAS( final long offset, final Object old, final Object nnn ) {
    return Unsafe.instance.compareAndSwapObject(this, offset, old, nnn );
  }
  ...
  private static final class CHM implements Serializable {
    ...
    private boolean CAS_key( int idx, long   old, long   key ) {
      return Unsafe.instance.compareAndSwapLong  ( _keys, rawIndex(_keys, idx), old, key );
    }
    private boolean CAS_val( int idx, Object old, Object val ) {
      return Unsafe.instance.compareAndSwapObject( _vals, rawIndex(_vals, idx), old, val );
    }
    ...
    private CHM resize() {
      ...
      if( r >= 2 && megs > 0 ) { // Already 2 guys trying; wait and see
        ...
        try { Thread.sleep(8*megs); } catch( Exception e ) { /*empty*/}
      }
